...
DynamicArray* dyNew(int capacity)
{
	DynamicArray* array;
	assert(capacity > 0);
	array = malloc(sizeof(struct DynamicArray));
	assert(array != 0);
    init(array, capacity);
    return array;
}
...
void adjustHeap(DynamicArray* heap, int last, int position, compareFunction compare)
{
	...
	if (rightChild < last)
	{
		smallChild = indexSmallest(heap, leftChild, rightChild, compare);
		if (compare(heap->data[smallChild], heap->data[position]) == -1)
		{
...
int indexSmallest(struct DynamicArray * v, int i, int j, compareFunction compare) {
	/* return index of smallest element */
	if (compare(v->data[i], v->data[j]) == -1)
		return i;
	return j;
}
...
void buildHeap(DynamicArray* heap, compareFunction compare)
{
	assert(heap != 0);
	int cur;
	for ((cur = dySize(heap) / 2 - 1); cur >= 0; cur--)
	{
		adjustHeap(heap, heap->size, cur, compare);
	}
}
...
void dyHeapAdd(DynamicArray* heap, TYPE value, compareFunction compare)
{
	...
	while (compare(value, parentVal) == -1 && pos != 0)
	{
